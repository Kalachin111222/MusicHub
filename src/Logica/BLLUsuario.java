...
public class BLLUsuario {
    ...
    public static boolean actualizarPerfil(String nombre, String email,
        ...
        usuarioActual.setFechaDeNacimiento(fechaNacimiento);
    ...
    private static boolean validarFechaNacimiento(LocalDate fecha) {
        ...
        if (fecha.isAfter(LocalDate.now().minusYears(13))) {
            mostrarError("Debes tener al menos 13 años.");
            return false;
        }

        // 3. Edad máxima (Ej. 120 años) - Opcional pero recomendado
        if (fecha.isBefore(LocalDate.now().minusYears(120))) {
            mostrarError("La fecha de nacimiento no es válida (demasiado antigua).");
            return false;
        }
